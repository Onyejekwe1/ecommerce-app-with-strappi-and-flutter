...
ThunkAction<AppState> getProductsAction = (Store<AppState> store) async {
  http.Response response = await http.get('http://192.168.1.105:1337/products');

  final List<dynamic> responseData = json.decode(response.body);

  List<Product> products =
      responseData.map((j) => Product.fromJson(j)).toList();
...
ThunkAction<AppState> toggleCartProductAction(Product cartProduct) {
  return (Store<AppState> store) async {
    ...
    await http.put('http://192.168.1.105:1337/carts/${user.cartId}', body: {
...
ThunkAction<AppState> getCartProductsAction = (Store<AppState> store) async {
  ...
  http.Response response = await http.get(
      'http://192.168.1.105:1337/carts/${user.cartId}',
      headers: {'Authorization': 'Bearer ${user.jwt}'});
...
ThunkAction<AppState> getCardsAction = (Store<AppState> store) async {
  ...
  http.Response response =  await http.get('http://192.168.1.105:1337/card?$customerId');
